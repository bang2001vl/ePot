...
public class OrderTab_Success extends OrderTab {
    @Override
    public List<BillAdapterItemInfo> loadDataFromDB() {
        List<BillAdapterItemInfo> rs = null;
        DBControllerBill db = new DBControllerBill();
        rs = db.getBillCustomer_ByStatus(Authenticator.getCurrentUser().id, BillBaseDB.BillStatus.SUCCESS, lastIndex, step);
        db.closeConnection();
